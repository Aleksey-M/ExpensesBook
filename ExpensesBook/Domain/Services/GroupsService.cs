...
namespace ExpensesBook.Domain.Services
{
    ...
    internal class GroupsService : IGroupsService
    {
        ...
        public GroupsService(IGroupsRepository gRepo, ICategoriesRepository catRepo, IGroupDefaultCategoryRepository gDefCatRepo, IExpensesRepository expensesRepo)
        {
            _groupsRepo = gRepo;
        ...
        public async ValueTask<Group> AddGroup(string groupName, int? sortOrder, IEnumerable<Guid>? relatedCategories)
        {
            ...
            if (relatedCategories != null && relatedCategories.Any())
            {
                ...
                var relations = filtered.Select(id => new GroupDefaultCategory { Id = Guid.NewGuid(), GroupId = group.Id, CategoryId = id }).ToList();
        ...
        public async ValueTask UpdateGroup(Guid groupId, string? groupName, int? groupOrder, IEnumerable<Guid>? relatedCategories)
        {
            ...
            if (relatedCategories is not null)
            {
                ...
                relCateg = relatedCategories.Select(c => new GroupDefaultCategory { Id = Guid.NewGuid(), GroupId = groupId, CategoryId = c }).ToList();
