...
namespace ExpensesBook.Domain.Services
{
    ...
    internal class ExpensesService : IExpensesService
    {
        ...
        public async ValueTask<Expense> AddExpense(DateTimeOffset date, double amounth, string description, Guid categoryId, Guid? groupId)
        {
        ...
        public async ValueTask DeleteExpense(Guid expenseId, DateTimeOffset date) => await _expensesRepo.DeleteExpense(expenseId, date);
        ...
        public async ValueTask UpdateExpense(Guid expenseId, DateTimeOffset? date, DateTimeOffset oldDate, double? amounth, string? description, Guid? categoryId, Guid? groupId)
        {
        ...
        public async ValueTask<List<(int year, int month, string monthName)>> GetExpensesMonths() =>
            (await _expensesRepo.GetMonths())
                .Select(e => (e.year, e.month, new DateTime(e.year, e.month, 1).ToString("MMMM", System.Globalization.CultureInfo.CreateSpecificCulture("ru-RU"))))
                .OrderBy(ym => ym.year)
                .ThenBy(ym => ym.month)
                .ToList();

        public async ValueTask<List<(Expense item, Category category, Group? group)>> GetExpensesWithRelatedData(DateTimeOffset? startDate, DateTimeOffset? endDate, string? filter)
        {
            ...
            {
                var category = allCategories.SingleOrDefault(c => c.Id == exp.CategoryId) ?? throw new Exception($@"Category with Id='{exp.CategoryId}' does not exists");

                Group? group = null;
                if (exp is not null && exp.GroupId is not null)
                {
                    group = allGroups.SingleOrDefault(g => g.Id == exp.GroupId) ?? throw new Exception($@"Group with Id='{exp.GroupId}' does not exists");
                }
